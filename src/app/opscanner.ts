...
export class OpScanner {
   ...
   * @param boolean 
   */
  public queueScanner(start: number, max: number) {
      ...
      .then(maxHeight => {
        ...
        (async () => {
          await this.delay(300)
            .then(any => {
              this.scanForMessages(start, this.defaultLimit)
                .then(any => {
                  this.queueScanner((start + this.defaultLimit), max);
                })
            })
                          ...
                          if (vout.hasOwnProperty('value') && vout['value'] == 0) {
                            ...
                            if (vout.hasOwnProperty('scriptPubKey')) {
                              if (vout['scriptPubKey']['asm'] && vout['scriptPubKey']['asm'].substr(0, 9) == 'OP_RETURN') {
                                ...
                                let dehexed = this.hex2a(vout['scriptPubKey']['asm'].substr(10, vout['scriptPubKey']['asm'].length))

                                if (dehexed.substr(0, 4) == 'S08k' || dehexed.substr(0, 3) == '08k') {
                                  console.log(result['blocktime'])
                                  let date = new Date(result['blocktime'] * 1000).toLocaleString("en-US");
                                  console.log(date)
                                  console.log(dehexed)
                                }
  ...
  private decodeOpReturn(buffer: any, i: number) {

    try {

      // need to add test to avoid zip-bombs
